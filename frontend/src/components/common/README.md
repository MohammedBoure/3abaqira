...
- **`AuthLoginModal.jsx`**: Enterprise authentication modal and branch-scoped account switcher allowing immediate or credentialed login across Admin accounts (with full cross-branch movement authority) and dedicated branch accounts (restricted to Center or Rawda).
- **`ContextMenu.jsx`**: Sharp right-click context menu primitive (`rounded-none`, border-tinted) with automatic viewport boundary clamping, grouped action ribbons, shortcut hints, dismiss triggers (`Escape`, outside click, scroll), and icon integration for tables and interactive elements.
...
- **`StatCounterCard.jsx`**: High-density metric tile with soft architectural borders, colored indicator border-strip, dedicated flex containers with min-h guardrails to prevent text clipping/overlapping, monospaced tabular figures, and percentage delta badges.
