...
| [connection.py](file:///C:/Users/moham/Desktop/3abaqira/backend/database/base/connection.py) | `ConnectionManager` handling PostgreSQL / MySQL connection pools, thread-safe context managers, transparent SQLite and MySQL compatibility layers (`SQLiteCompatCursor`, `MySQLCompatCursor`, `MySQLCompatConnection` for `ILIKE`, `ON CONFLICT`, and `RETURNING` emulation), automatic fallback to SQLite when external databases are unreachable, and SQLAlchemy engine creation. |
...
| [schema_initializer.py](file:///C:/Users/moham/Desktop/3abaqira/backend/database/base/schema_initializer.py) | Executes table definitions, views, and indexes with cross-dialect adaptation (PostgreSQL, SQLite, and MySQL dialect transformations). Utilizes SHA256 checksum fingerprinting in `AppMetadata` to bypass redundant startup checks. |
